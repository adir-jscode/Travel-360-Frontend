...
/* ─────────────────── Main Page ──────────────────── */
export default function TripRequestsComponent({
  initialRequests,
}: {
  initialRequests: IJoinRequest[];
}) {
  ...
  const hookHasLoaded = pending.length + accepted.length + rejected.length > 0;
  const seedPending = useMemo(
    () => initialRequests.filter((r) => r.status === JoinRequestStatus.PENDING),
    [initialRequests],
  );
  const seedAccepted = useMemo(
    () =>
      initialRequests.filter((r) => r.status === JoinRequestStatus.ACCEPTED),
    [initialRequests],
  );
  const seedRejected = useMemo(
    () =>
      initialRequests.filter((r) => r.status === JoinRequestStatus.REJECTED),
    [initialRequests],
  );

  const effectivePending = hookHasLoaded ? pending : seedPending;
  const effectiveAccepted = hookHasLoaded ? accepted : seedAccepted;
  const effectiveRejected = hookHasLoaded ? rejected : seedRejected;
